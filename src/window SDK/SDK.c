...
DATAWORD PKGetTempPixel(int xpos, int ypos, int n)
{
	...
	if(temp_dat[n] == 0){
		beep();
		printf("Temporary Image does not exist\n");
		return OMA_NOEXIST;
	} 	
...
int PKSetPixel( int xpos, int ypos, DATAWORD value)
{
	...
	if (index >= header[NCHAN]*header[NTRAK]){
		beep();
		//printf("We were about to write to memory outside image allocation!\n");
		return OMA_RANGE;
	}
...
int SDK_Image_SetPixel(Image *ImPointer, short xpos, short ypos, DATAWORD value)
{
	...
	if (ImPointer == 0) {
		beep();
		printf("Pointer to image must be non-NULL\n");
		return OMA_NOEXIST;
	}
...
SDK_Image_CopyImage (Image *imptr, Image *destptr)	
{
	...
	if ((imptr == 0)||(destptr ==0)) {
		beep();
		printf("Pointer to SRC & DST images must be non-NULL\n");
		return OMA_NOEXIST;
	}
...
SDK_Image_XCorr (Image *src1ptr, Image *src2ptr, Image *destptr)	
{
	...
	if ( (fftwidth !=  src2ptr->width) || (fftheight != src2ptr->height) 
		...
		beep();
		printf("The SRC & DST images need to be the same size\n");
		return OMA_MISC;
	}
	...
	if (!is_power_2(fftwidth) || !is_power_2(fftheight)){
		beep();
		printf("We are still forcing 2^n requirement until the move to FFTW from FT2D\n");
		return OMA_RANGE;
	}
...
SDK_Image_3PtEstimatorX (Image *imptr, int xpos, int ypos)	
{
	...
	if (imptr == 0) {
		beep();
		printf("Pointer to source image must be non-NULL\n");
		return OMA_NOEXIST;
	}
...
SDK_Field_2Buffer (Field *myfield)	
{
	...
	peaknew = (int*)malloc(no_of_velx * no_of_vely * peaks_to_save * sizeof(int));
...
SDK_Field_2NthPeak (Field *myfield, short n)	
{
	...
	if ((no_of_velx != myfield->width)||(no_of_vely != myfield->height)) {
		beep();
		printf("Field size and existing PIV buffer are not the same size\n");
		return OMA_RANGE;
	}
	...
	if ((n <1)||(n >= peaks_to_save)) {
		beep();
		printf("Your peak number is out of range:%d\n", peaks_to_save);
		return OMA_RANGE;
	}
...
PKPixel2Vector(int x,int y)
{
	...
	{
		beep();
		printf("Must have a valid PIV image first\n");
		return (retVec);
	}
